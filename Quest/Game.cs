...
namespace Quest
{
    class Game
    {
        ...
        public void Move(Direction moveDirection, Random random)
        {
        ...
        private void HitPlayer(int maxDamage, Random random)
        {
        ...
        {
            player.Attack(attackDirection, random);
            foreach (Enemy enemy in Enemies)
                enemy.Move(random);
        }

        public bool CheckPlayerInventory(string item) //item būs Weapon Dictionary?
        {
            return player.Weapons.Contains(item);
        }
        ...
        private Point GetRandomLocation(Random random)
        {
            return new Point(boundaries.Left + random.Next(boundaries.Right / 10 -
                boundaries.Left / 10) * 10, boundaries.Top + random.Next(boundaries.Bottom / 10 - boundaries.Top / 10) * 10);
        }
